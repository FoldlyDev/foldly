...
function checkFileDirectives(filePath) {
  ...
  if (hasClerkServerImport && !hasUseServer && !filePath.includes('/api/') && !filePath.includes('page.tsx') && !filePath.includes('layout.tsx')) {
    VIOLATIONS.push({
      file: filePath,
      type: 'MISSING_USE_SERVER',
      description: 'File imports @clerk/nextjs/server but missing "use server" directive'
    });
  }
...
function checkReactQueryPatterns(filePath) {
  ...
  const hasDirectServerImport = content.match(/import.*from.*['"].*\/actions['"]/);
  
  if (hasUseQuery && hasDirectServerImport && !content.includes('dynamic import')) {
    VIOLATIONS.push({
      file: filePath,
      type: 'STATIC_SERVER_IMPORT_IN_QUERY',
      description: 'React Query hook has static server action import (should use dynamic import)'
    });
  }
}
