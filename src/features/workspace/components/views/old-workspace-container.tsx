...
} from '@/features/workspace/stores/workspace-modal-store';
...
export function WorkspaceContainer() {
  ...
  const dropCallbacks: DropOperationCallbacks = useMemo(
    () => ({
      onReorder: async (
        ...
        try {
          ...
          if (result.success) {
            ...
            eventBus.emitNotification(
              NotificationEventType.WORKSPACE_ITEMS_REORDER_SUCCESS,
              {
                batchId: reorderBatchId,
                totalItems: draggedItemIds.length, // Use actual dragged items count
          ...
          eventBus.emitNotification(
            NotificationEventType.WORKSPACE_ITEMS_REORDER_ERROR,
            {
              batchId: reorderBatchId,
              totalItems: draggedItemIds.length, // Use actual dragged items count
              completedItems: 0,
      ...
      onMove: async (
        ...
        eventBus.emitNotification(
          NotificationEventType.WORKSPACE_ITEMS_MOVE_START,
          {
            batchId: moveBatchId,
        ...
        try {
          ...
          eventBus.emitNotification(
            NotificationEventType.WORKSPACE_ITEMS_MOVE_SUCCESS,
            {
              batchId: moveBatchId,
              totalItems: itemIds.length,
          ...
          eventBus.emitNotification(
            NotificationEventType.WORKSPACE_ITEMS_MOVE_ERROR,
            {
              batchId: moveBatchId,
              totalItems: itemIds.length,
              completedItems: 0,
  ...
  const contextMenuProvider: ContextMenuProvider = useCallback(
    (item: TreeItem, itemInstance: any) => {
      ...
      const isItemSelected = selectedTreeItems.some(
        (si: any) => si.getId() === item.id
      );
      const isMultipleSelection =
        isItemSelected && selectedTreeItems.length > 1;
          ...
          if (isMultipleSelection) {
            ...
            itemsToDeleteArray = selectedTreeItems.map((selectedItem: any) => {
              ...
              return {
                ...
                name:
                  selectedItem.getItemName?.() || itemData?.name || 'Unknown',
      ...
      if (isFolder(item) && !isMultipleSelection) {
          ...
          onClick: async () => {
            ...
            try {
              ...
              if (result.success && result.data) {
                ...
                eventBus.emitNotification(
                  NotificationEventType.WORKSPACE_FOLDER_CREATE_SUCCESS,
                  {
        ...
        menuItems.push({
          ...
          onClick: async () => {
            try {
              const result = await generateLinkFromFolderAction({
                folderId: item.id,
              });
              ...
              if (result.success && result.data) {
                ...
                eventBus.emitNotification(
                  NotificationEventType.LINK_GENERATE_SUCCESS,
                  {
                ...
                eventBus.emitNotification(
                  NotificationEventType.LINK_GENERATE_ERROR,
                  {
