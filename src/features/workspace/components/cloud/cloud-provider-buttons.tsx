...
export function CloudProviderButtons({ className }: CloudProviderButtonsProps) {
  ...
  const connectionStatus = useMemo(() => {
    ...
    return {
      google: user.externalAccounts.some(account => account.provider === 'oauth_google'),
      microsoft: user.externalAccounts.some(account => account.provider === 'oauth_microsoft')
    };
  ...
  const handleConnectGoogle = async () => {
    ...
    if (!user) {
      showWorkspaceError('cloud_connection_failed', {
        itemName: 'Google Drive',
        itemType: 'cloud_provider'
    ...
    try {
      ...
      await connectGoogleWithReverification();
        ...
        if (isClerkRuntimeError(error) && isReverificationCancelledError(error)) {
          showWorkspaceError('cloud_verification_cancelled', {
            itemName: 'Google Drive',
            itemType: 'cloud_provider'
  ...
  return (
    <div className={cn('flex flex-col gap-2', className)}>
      ...
      <Button
        ...
        title={connectionStatus.google ? 'Google Drive Connected' : 'Connect Google Drive'}
      >
        {isConnecting === 'google' ? (
      ...
      <Button
        ...
        title={connectionStatus.microsoft ? 'OneDrive Connected' : 'Connect OneDrive'}
      >
