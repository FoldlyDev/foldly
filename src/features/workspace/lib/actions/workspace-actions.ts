...
export async function getWorkspaceByUserId(): Promise<ActionResult<Workspace>> {
  try {
    ...
    if (!workspace) {
      logger.info('Workspace not found, creating for user', {
        userId: sanitizedUserId,
      });

      // Import user workspace service for creation
      const { userWorkspaceService } = await import(
        '@/features/users/services/user-workspace-service'
      );

      // Check if user exists first
      const user = await userWorkspaceService.getUserById(sanitizedUserId);
      if (!user) {
        logger.warn('User not found during workspace creation attempt', {
          userId: sanitizedUserId,
        });
 ...
 */
export async function checkWorkspaceStatusAction(): Promise<
 ...
 */
export async function createWorkspaceAction(): Promise<
  ...
  try {
    ...
    if (!user) {
      ...
      if (!sessionClaims?.email) {
        ...
        return createErrorResponse(
          'User profile not complete. Please try again.',
