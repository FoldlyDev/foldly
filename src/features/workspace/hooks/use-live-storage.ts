...
const useLiveStorageStore = create<LiveStorageState & LiveStorageActions>((set, get) => ({
  // Initial state
  baseUsage: 0,
  uploadingBytes: 0,
  completedBytes: 0,
...
export function useLiveStorage(): LiveStorageData & LiveStorageActions {
  ...
  const uploadProgress = totalBytes > 0 ? (uploadedBytes / totalBytes) * 100 : 0;
  ...
  const realtimeUsage = state.baseUsage + state.completedBytes + realtimeUploadedBytes;
...
export function useFileUploadProgress(fileId: string) {
  const fileProgress = useLiveStorageStore((state) => state.fileProgress.get(fileId));
