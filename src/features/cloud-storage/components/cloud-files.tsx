...
function Files({ className, children, ...props }: FilesProps) {
  return (
    <FilesPrimitive className={cn('p-2 w-full', className)} {...props}>
      <FilesHighlightPrimitive className="bg-gray-100 dark:bg-white/5 rounded-lg pointer-events-none">
        {children}
      ...
      <FolderTriggerPrimitive className="w-full text-start">
        <FolderHighlightPrimitive>
          <FolderPrimitive className="flex items-center justify-between gap-2 p-2 pointer-events-none">
            ...
            >
              <FolderIconPrimitive
                closeIcon={<FolderIcon className="size-4.5 text-gray-600 dark:text-gray-400" />}
                openIcon={<FolderOpenIcon className="size-4.5 text-gray-600 dark:text-gray-400" />}
              />
              <FileLabelPrimitive
...
function FolderPanel(props: FolderPanelProps) {
  return (
    <div className="relative ml-6 before:absolute before:-left-3 before:top-0 before:bottom-0 before:w-px before:bg-gray-300 dark:before:bg-white/10">
    ...
    <FileHighlightPrimitive>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <FileLabelPrimitive className={cn('text-sm text-gray-900 dark:text-gray-100', className)} {...props}>
            {children}
          </FileLabelPrimitive>
        </div>
