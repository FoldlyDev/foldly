...
export const DATABASE_ERROR_MESSAGES = {
  ...
  [DATABASE_ERROR_CODES.LINK_NOT_FOUND]: 
    'The requested link was not found.',
  [DATABASE_ERROR_CODES.VALIDATION_ERROR]: 
 ...
 */
export function detectConstraintViolation(error: Error): DatabaseErrorCode | null {
  const message = error.message.toLowerCase();
  
  // Trigger-based slug consistency errors
  if (message.includes('all links must use the same slug') || 
  ...
  if (message.includes('links_slug_topic_idx') || message.includes('duplicate key')) {
