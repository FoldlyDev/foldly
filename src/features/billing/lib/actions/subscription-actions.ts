...
export async function getCurrentUserPlanConfigAction() {
  try {
    ...
    const planMetadataResult = await billing.integration.getPlanUIMetadata(currentPlanResult.data.currentPlan);
...
export async function getRealTimeStorageUsageAction() {
  try {
    ...
    const percentage = storageInfo.storageLimitBytes > 0 
      ? Math.round((storageInfo.storageUsedBytes / storageInfo.storageLimitBytes) * 100) 
      : 0;
...
export async function getUserBillingDataAction() {
  try {
    ...
    const { BillingAnalyticsService } = await import('@/lib/services/billing/billing-analytics-service');
    const billingData = await BillingAnalyticsService.getUserBillingData(userId);
...
export async function getBillingOverviewAction(currentPlan: string = 'free') {
  try {
    ...
    const { BillingAnalyticsService } = await import('@/lib/services/billing/billing-analytics-service');
    const overviewData = await BillingAnalyticsService.getBillingOverview(userId, currentPlan);
...
export async function getStorageUsageAction(currentPlan: string = 'free') {
  try {
    ...
    const { BillingAnalyticsService } = await import('@/lib/services/billing/billing-analytics-service');
    const storageData = await BillingAnalyticsService.getStorageUsage(userId, currentPlan);
...
export async function getBillingIntegrationStatusAction() {
  try {
    ...
    const { getUserStorageDashboard } = await import('@/lib/services/storage/storage-tracking-service');
    const storageInfo = await getUserStorageDashboard(userId, userData.clerkPlan.currentPlan);
...
export async function getUserStorageStatusAction() {
  try {
    ...
    // Get storage info from storage service
    const { getUserStorageDashboard, formatBytes } = await import('@/lib/services/storage/storage-tracking-service');
    const storageInfo = await getUserStorageDashboard(userId, currentPlan);
    ...
    return { 
      ...
      data: {
        ...
        limitFormatted: isUnlimited ? 'Unlimited' : formatBytes(storageInfo.storageLimitBytes),
        percentage: Math.min(percentage, 100),
        isNearLimit: percentage > 80,
        isOverLimit: percentage > 100,
