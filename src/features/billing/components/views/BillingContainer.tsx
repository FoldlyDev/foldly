...
import {
  ...
  useTabSwitchingReadiness,
  usePricingTableReadiness,
} from '../../hooks/use-coordinated-billing-loading';
...
export function BillingContainer({
  ...
  const [activeSection, setActiveSection] = useState<'overview' | 'pricing'>(
    'overview'
  );

  // Coordinated loading state management
  const coordinatedLoading = useCoordinatedBillingLoading();
  const { isReady: canSwitchTabs, shouldShowTabLoading } =
    useTabSwitchingReadiness();
  const {
    isReady: canRenderPricingTable,
    shouldShowClerkLoading,
  ...
  const preserveActiveSection = useCallback(
  ...
  useEffect(() => {
    if (user && isLoaded && planConfig) {
      ...
      const isFromClerkCheckout =
        urlParams.has('success') || activeSection === 'pricing';
        ...
        const showSuccessNotification = setTimeout(() => {
          ...
          if (urlParams.has('success')) {
            window.history.replaceState(
              {},
              '',
              window.location.pathname + '?tab=pricing'
            );
          }
        }, 500);
  ...
  const billingData = useMemo(() => {
    ...
    const storageUsedGB =
      Math.round((storageUsedBytes / 1024 ** 3) * 100) / 100;
                    ...
                    <div>
                      ...
                      <p className='text-[var(--neutral-600)] text-sm'>
                        {planConfig?.planKey === 'free'
                          ...
                          : `Active subscription - $${planConfig?.monthlyPrice}/month`}
                      </p>
                    </div>
              ...
              {planConfig?.planKey !== 'business' && (
                <div className='bg-[var(--primary-subtle)] rounded-lg border border-[var(--primary)]/20 p-6'>
                  ...
                  <p className='text-[var(--neutral-600)] text-sm mb-4'>
                    Upgrade your plan to access more storage, advanced features,
                    and priority support.
                  </p>
                  <Button onClick={() => preserveActiveSection('pricing')}>
                    View Plans
                  </Button>
                </div>
              )}
            ...
            <div id='pricing-section'>
              <div className='mb-6'>
                ...
                <p className='text-[var(--neutral-600)]'>
                  Upgrade or downgrade your subscription at any time. Changes
                    ...
                    <AlertDescription className='text-blue-800'>
                      <div className='inline-flex items-center gap-2'>
                        ...
                        <span className='text-xs'>
                          ({coordinatedLoading.loadingPhase})
                        </span>
                      </div>
                    </AlertDescription>
                  </Alert>
              ...
              <ClerkLoading>
                <ClerkPricingTableSkeleton loadingMessage='Initializing Clerk billing system...' />
              </ClerkLoading>

              <ClerkLoaded>
                <Suspense
                  fallback={
                    <ClerkPricingTableSkeleton loadingMessage='Loading pricing data...' />
                  }
                >
