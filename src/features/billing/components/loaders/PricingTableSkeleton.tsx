              ...
              <div className='flex justify-center mb-4'>
                ...
                >
                  <Loader2
                    className={`
                      ${
                        config.iconSize === 'w-12 h-12'
                          ? 'w-5 h-5'
                          : config.iconSize === 'w-20 h-20'
                            ? 'w-8 h-8'
                            : 'w-6 h-6'
                      } 
              ...
              <div className={config.spacing}>
                {[...Array(config.featuresCount)].map((_, j) => (
                  <div key={j} className='flex items-center gap-3'>
                    ...
                    <Skeleton
                      className={`h-3 ${j % 2 === 0 ? 'flex-1' : 'w-3/4'}`}
                    />
                  </div>
                ))}
              </div>
...
export const ClerkPricingTableSkeleton: React.FC<
  Pick<PricingTableSkeletonProps, 'loadingMessage'>
...
export const QuickPricingTableSkeleton: React.FC = () => (
  <PricingTableSkeleton variant='compact' showLoadingMessage={false} />
);

export const DetailedPricingTableSkeleton: React.FC<
  Pick<PricingTableSkeletonProps, 'loadingMessage'>
> = ({ loadingMessage = 'Loading subscription details...' }) => (
  <PricingTableSkeleton variant='detailed' loadingMessage={loadingMessage} />
);
