    ...
    <div className="max-w-6xl mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Test the corrected plan detection logic</p>
        </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <Badge variant={clerkSubscription.isProTier ? 'default' : 'secondary'}>
            ...
            <div className="flex items-center gap-2">
              ...
              <Badge variant={clerkSubscription.isLoaded ? 'default' : 'secondary'}>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2 text-sm">
                <div>Custom Branding: {clerkSubscription.hasCustomBranding ? '✅' : '❌'}</div>
                <div>Premium Links: {clerkSubscription.hasPremiumShortLinks ? '✅' : '❌'}</div>
                <div>Password Links: {clerkSubscription.hasPasswordProtectedLinks ? '✅' : '❌'}</div>
                <div>Priority Support: {clerkSubscription.hasPrioritySupport ? '✅' : '❌'}</div>
              </div>
            </div>
          </CardContent>
        </Card>
            ...
            {debugInfo ? (
              <>
                <div className="flex items-center gap-2">
                  ...
                  <Badge className={getPlanColor(debugInfo.debugInfo.planDetection.detectedPlan)}>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Badge variant={debugInfo.quickProCheck.isPro ? 'default' : 'secondary'}>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Badge variant={debugInfo.summary.hasClerkBilling ? 'default' : 'secondary'}>
                ...
                <div>
                  ...
                  <div className="space-y-1 text-sm">
                    {Object.entries(debugInfo.quickProCheck.rawClerkChecks).map(([key, value]) => (
              ...
              <div className="text-center py-4">
                ...
                <p className="mt-2 text-sm text-muted-foreground">Loading debug info...</p>
              </div>
            ) : (
        ...
        <Card>
          ...
          <CardContent>
            <ul className="space-y-2">
              {debugInfo.summary.recommendations.map((rec: string, index: number) => (
                <li key={index} className="flex items-start gap-2">
                  <span className="text-muted-foreground">•</span>
                  <span className={rec.includes('✅') ? 'text-green-700' : rec.includes('⚠️') ? 'text-yellow-700' : ''}>{rec}</span>
                </li>
              ))}
            </ul>
          </CardContent>
          ...
          <CardContent>
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <p>Storage Limit: {debugInfo.databasePlanConfig.storageLimit}GB</p>
              </div>
              <div>
                <h4 className="font-medium">Active Features</h4>
                <div className="space-y-1 text-sm">
                  {debugInfo.databasePlanConfig.features.map((feature: string) => (
      ...
      <Card className="bg-muted">
        <CardContent className="pt-6">
          <p className="text-sm text-muted-foreground text-center">
            Last updated: {debugInfo?.timestamp ? new Date(debugInfo.timestamp).toLocaleString() : 'Not available'}
          </p>
        </CardContent>
      </Card>
    </div>
