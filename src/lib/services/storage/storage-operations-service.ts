...
export class StorageService {
  ...
  async deleteFile(
    ...
    try {
      ...
      console.log(`🗑️ ATTEMPTING_STORAGE_DELETE: Path="${path}" Bucket="${bucketName}"`);
      ...
      if (listError) {
        ...
        console.warn(`⚠️ FILE_NOT_FOUND_IN_STORAGE: Path="${path}" in bucket "${bucketName}"`);
        // Continue with deletion attempt anyway in case of listing issues
      }
      ...
      if (error) {
        console.error(`❌ STORAGE_DELETE_FAILED: Path="${path}" Bucket="${bucketName}" Error:`, error);
        
        // If file doesn't exist, consider it a success (already deleted)
        if (error.message?.includes('not found') || error.message?.includes('does not exist')) {
          console.log(`⚠️ FILE_ALREADY_DELETED: Path="${path}" (treating as success)`);
          return { success: true, data: undefined };
        }
      ...
      console.log(`✅ FILE_DELETED_FROM_STORAGE: Path="${path}" Bucket="${bucketName}"`);
      return { success: true, data: undefined };
    } catch (error) {
      console.error(`❌ UNEXPECTED_STORAGE_DELETE_ERROR: Path="${path}"`, error);
      return { success: false, error: (error as Error).message };
    }
  }
