...
import type { UserStorageInfo, StorageValidationResult } from '@/lib/services/storage';
...
export const STORAGE_QUERY_KEYS = {
  ...
  validation: (userId: string, fileSize: number) => ['storage', 'validation', userId, fileSize],
} as const;
...
export const useStorageDashboard = (planKey: string = 'free') => {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { getUserStorageDashboardAction } = await import('@/lib/services/storage/storage-actions');
...
export const useStorageBreakdown = () => {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // Dynamic import to avoid client/server boundary issues
      const { getStorageBreakdownAction } = await import('@/lib/services/storage/storage-actions');
 ...
 */
export const useUploadValidation = (fileSize: number, planKey: string = 'free') => {
  const { user } = useUser();
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // Dynamic import to avoid client/server boundary issues
      const { validateUploadAction } = await import('@/lib/services/storage/storage-actions');
...
export const useUploadWithTracking = (planKey: string = 'free') => {
  ...
  return useMutation({
    mutationFn: async (params: {
      ...
      // Dynamic import to avoid client/server boundary issues
      const { uploadFileAction } = await import('@/lib/services/storage/storage-actions');
...
export const useDeleteWithTracking = () => {
  ...
  return useMutation({
    mutationFn: async (fileId: string) => {
      ...
      // Dynamic import to avoid client/server boundary issues
      const { deleteFileAction } = await import('@/lib/services/storage/storage-actions');
...
export const useStorageSync = () => {
  ...
  return useMutation({
    mutationFn: async () => {
      ...
      // Dynamic import to avoid client/server boundary issues
      const { syncStorageAction } = await import('@/lib/services/storage/storage-actions');
 ...
 */
export const useStorageState = (fileSize?: number, planKey: string = 'free') => {
