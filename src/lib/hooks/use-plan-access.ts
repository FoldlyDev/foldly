...
export const PLAN_QUERY_KEYS = {
  ...
  featureAccess: (userId: string, feature: string) => ['plan', 'feature', userId, feature],
  upgradeEligibility: (userId: string, targetPlan: string) => ['plan', 'upgrade', userId, targetPlan],
} as const;
...
export function useCurrentUserPlan() {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { getCurrentUserPlanAction } = await import('@/features/billing/lib/actions');
...
export function useUserPlanDetails() {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // Dynamic import to avoid client/server boundary issues
      const { getUserPlanDetailsAction } = await import('@/features/billing/lib/actions');
...
export function usePlanConfig(planKey: 'free' | 'pro' | 'business') {
  return useQuery({
    ...
    queryFn: async () => {
      // Dynamic import to avoid client/server boundary issues
      const { getCurrentUserPlanConfigAction } = await import('@/features/billing/lib/actions');
...
export function useAllPlans() {
  return useQuery({
    ...
    queryFn: async () => {
      // Dynamic import to avoid client/server boundary issues
      const { getActivePlansAction } = await import('@/features/billing/lib/actions');
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (!integratedDataResult.success) {
        throw new Error(integratedDataResult.error || 'Failed to get plan data');
      }
...
export function useUpgradeEligibility(targetPlan: 'pro' | 'business') {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const canUpgrade = integratedDataResult.data.upgradeOptions.includes(targetPlan);
...
export function usePlanState() {
  ...
  const isLoading = currentPlan.isLoading || planDetails.isLoading || features.isLoading;
  const isError = currentPlan.isError || planDetails.isError || features.isError;
