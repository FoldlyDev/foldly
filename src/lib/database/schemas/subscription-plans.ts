...
export const subscriptionPlans = pgTable(
  'subscription_plans',
  {
    ...
    featureDescriptions: jsonb('feature_descriptions').$type<Record<string, string>>(), // Detailed feature explanations
  ...
  table => ({
    subscriptionPlansPlanKeyIdx: index('idx_subscription_plans_plan_key').on(table.planKey),
    subscriptionPlansActiveIdx: index('idx_subscription_plans_active').on(table.isActive),
    subscriptionPlansSortOrderIdx: index('idx_subscription_plans_sort_order').on(table.sortOrder),
  })
);
