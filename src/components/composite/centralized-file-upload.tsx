  ...
  const files = React.useMemo(() => {
    if (externalFiles.length > 0) {
      console.log('📁 [CENTRALIZED-UPLOAD] Using external files:', {
        ...
        previews: externalFiles.map(f =>
          'preview' in f ? (f as any).preview : 'none'
        ),
      });
  ...
  const handleFileChange = (newFiles: File[]) => {
    ...
    if (maxFileSize) {
      ...
      if (oversizedFiles.length > 0) {
        ...
        const message =
          oversizedFiles.length === 1 && firstFile
    ...
    if (allowedFileTypes && allowedFileTypes.length > 0) {
      ...
      if (invalidFiles.length > 0) {
        ...
        const allowedTypeNames = allowedFileTypes
          .map(t => fileTypeMap[t] || t)
          .join(', ');
        const firstInvalidFile = invalidFiles[0];
        const message =
          invalidFiles.length === 1 && firstInvalidFile
    ...
    if (multiple) {
      ...
      if (externalFiles.length > 0) {
        ...
        console.log(
          '📊 [CENTRALIZED-UPLOAD] External mode - passing new files only:',
          {
        ...
        console.log(
          '📊 [CENTRALIZED-UPLOAD] Internal mode - accumulating files:',
          {
              ...
              {customEmptyState || (
                <>
                  ...
                  <div className='text-center space-y-1 sm:space-y-2'>
                    ...
                    >
                      {isDragActive ? 'Perfect! Drop them here' : uploadText}
                    </h3>
                    <p
                      className={cn(
