...
export const useLinksState = () => {
  ...
  const computedValues = useMemo(() => {
    ...
    // ✅ Pin base link to top: Separate base link from topic links BEFORE sorting
    const baseLinkInFiltered = filteredLinks.find(
      link => !link.topic || link.topic.trim() === ''
    );
    const topicLinksToSort = filteredLinks.filter(
      link => link.topic && link.topic.trim() !== ''
    );

    // ✅ Apply sorting ONLY to topic links (base link always stays first)
    const sortedTopicLinks = [...topicLinksToSort].sort((a, b) => {
      switch (sortOption) {
        case 'created_desc':
          return (
    ...
    // ✅ Combine: Base link first, then sorted topic links
    const finalSortedLinks = baseLinkInFiltered
      ? [baseLinkInFiltered, ...sortedTopicLinks]
      : sortedTopicLinks;
    ...
    return {
      hasBaseLink,
      baseLink,
      filteredLinks: finalSortedLinks,
      baseLinkInFiltered,
      topicLinks: finalSortedLinks.filter(
        link => link.topic && link.topic.trim() !== ''
      ),
      linkStats,
